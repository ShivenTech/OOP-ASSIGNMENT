...
public class Main {
    ...
    static JsonDataHandler dataHandler = new JsonDataHandler();
    static String customerFile = "customers.json";
   ...
   public static void main(String[] args) {
        // 1. Uncomment this so you actually have cars to rent!
        try {
        // Load existing customers from the file
            customerList = (ArrayList<Customer>) dataHandler.load(customerFile);
        } catch (IOException e) {
            System.out.println("Starting with fresh customer list.");
        }
        initializeDummyData();
        ...
        while (running) {
            ...
            try {
                ...
                switch (choice) {
                    case 1:
                        registerCustomer(); // Now correctly linked to option 1
                        break;
                    case 2:
                        rentCar(); 
                        break;
 ...
 // ==========================================
    ...
    public static void registerCustomer() {
        ...
        Customer newCust = new Customer(id, name, license, contact, email);
        
        // Add to the local list for this session
        customerList.add(newCust);

        // check if the file saved or not
        try {
            dataHandler.save(customerList, customerFile);
            System.out.println("Data saved to JSON file successfully!");
        } catch (IOException e) {
            System.out.println("Error saving to file: " + e.getMessage());
        }
        
        // Use the method from your Customer class to save to file
        newCust.saveToFile();
        
        System.out.println("Customer registered successfully in system memory.");
    }
